...
class WeatherSegment(ThreadedSegment):
	...
	def update(self, old_weather):
		...
		if not self.url:
			...
			self.url = 'http://api.openweathermap.org/data/2.5/weather?' + urllib_urlencode({
				'q': self.location})
	...
	def render(self, weather, icons=None, unit='C', temp_format=None, temp_coldest=-30, temp_hottest=40, **kwargs):
		...
		temp, weather_icon = weather
		icon = (icons or {}).get(weather_icon, weather_conditions_icons.get(weather_icon, weather_icon))
		...
		return [
			...
			},
			{
				...
				'highlight_group': ['weather_temp', 'weather'],
				'divider_highlight_group': 'background:divider',
			},
		]
...
:param dict icons:
	dict for overriding default icons, e.g. ``{'13d' : u'❆'}``. See http://openweathermap.org/weather-conditions for list of icon names
